...
MyReg <- function(data) {
  ...
  dev.new()
  par(fig=c(0,0.7,0,1), new=FALSE)
  plot(pred~stdresid, main="Standard residuals VS predicted values",
       xlab="Studentized residuals", ylab="Fitted response values", 
       pch=21, cex=1, col="black") 
  abline(v=c(-2,0,2),col="saddlebrown",lty=c(2,1,2))
  rug(stdresid,side=1,col="lightblue4")
  for (i in 1:nrow(ex2)) {
    text(ex2$stdresid[i], ex2$pred[i], ex2$number[i], col="darkgreen", pos=2)
    points(ex2$stdresid[i], ex2$pred[i], pch=19, cex=2, col="tomato")
  }
  par(fig=c(0.7,1,0,1),new=TRUE)
  boxplot(stdresid,col="green",main="Boxplot of Standard residuals")
  for (i in 1:6) {
    text(0.7,summary(stdresid)[i],round(summary(stdresid)[i],3),col="hotpink4")
  }
  dev.new()
  par(fig=c(0,1,0,1),new=FALSE)
  plot(abs.stdresid~pred,
  ...
  qqnorm(stdresid) ; qqline(stdresid)
  dev.new()
  u <- seq(min(stdresid),max(stdresid),0.001)
  hist(stdresid,probability=T,col="bisque",main="Normality of standard residuals")
  lines(u,dnorm(u),lwd=2,lty=1,col="darkkhaki")
  test.res <- shapiro.test(stdresid)
  print(test.res)
